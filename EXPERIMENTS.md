...
## 실험 46: quick 데모 모드 도입 — 부팅 완주 41분 → 약 5분

**날짜:** 2026-07-30

**가설:** 전체 데모 완주에 2,442초(41분)가 걸려 시연 영상 촬영도, 저장소를
처음 받은 사람의 동작 확인도 불가능하다. 실험 42 로그를 구간별로 분해하면
커널 기능 데모 전체는 167초(7%)뿐이고 `bench_pe4::run()` 10회가 2,275초
(93%)를 차지한다. 벤치마크 워크로드만 줄인 모드를 따로 두면, 기존 실험
수치의 비교 가능성을 깨지 않으면서 완주 시간을 크게 줄일 수 있을 것이다.

**측정 환경:** Apple M4 Pro, QEMU 11.0.3, `-M q35 -smp 4`, debug 빌드.

**방법:** cargo feature `quick-demo`를 추가하고 `make run`(quick) /
`make run-full`(전체)로 분리. 부팅 배너에 현재 모드를 출력해 quick 모드
수치가 기록된 실험값과 혼동되지 않게 했다.

**발견 1 — 최초 원인 지목이 틀렸다 (`HOG_TARGET_ITERS`는 무관):**
처음에는 `HOG_TARGET_ITERS = 20_000_000`이 시간을 지배한다고 보고 1/10로
줄였다. **효과가 전혀 없었다.** 로그를 다시 보니 `hog완료=4tick` — hog는
약 0.2초 만에 목표를 채우고, 그 뒤로는 다른 요인이 시간을 쓰고 있었다.
"구간(bench_pe4)"은 맞게 짚었지만 "그 구간 안의 원인"을 확인 없이 추정한
오류다(실험 45에서 얻은 교훈을 곧바로 또 어겼다).

**발견 2 — 진짜 원인은 잘못 계산된 타임아웃:**
```rust
// 타임아웃 (starvation 등으로 무한 대기 방지, ~20초분 틱)
if tick - start_tick > 4000 { break; }
```
타이머는 약 18틱/초이므로 4000틱은 20초가 아니라 **약 222초**다. 주석의
계산이 10배 넘게 틀려 있었다. PE=ON 조건에서는 컨텍스트 스위치가 폭증해
키 입력 표본을 다 못 채우고(실측 n=20/40) 매번 이 한도까지 끌려갔다.
222초 × 10회 ≈ 37분이 여기서 나왔다.

**발견 3 — 두 번째 병목: 시리얼 출력량:**
벤치마크를 줄인 뒤에도 wall clock이 줄지 않아 다시 보니, 전체 로그
106,404줄 중 **104,545줄(4.7MB)이 BETA-X 3/4 두 데모**에서 나오고 있었다
(`[input-app]` 50,923줄, `[gfx]` 35,746줄, `[wm-ipc]` 17,876줄). UART가
바이트당 수십 µs를 바쁜 대기로 소모하므로 출력에만 수 분이 걸린다.
**guest 시간(TICK)만으로 구간을 분해하면 이 병목은 보이지 않는다** —
해당 구간의 guest 시간은 10초 남짓이기 때문이다. 처음 분해에서 놓친 이유.

**발견 4 — Makefile이 낡은 ISO를 그대로 남긴다:**
측정이 두 번이나 무효였다. cargo가 "fresh"로 판단해 바이너리를 다시 쓰지
않으면 커널 ELF의 mtime이 예전 그대로 남고, `$(ISO)`가 자기보다 오래된
ELF를 보고 갱신을 건너뛴다. 결과적으로 **수정이 반영되지 않은 ISO로
부팅하면서 "변경이 효과가 없다"고 잘못 판단**했다. `kernel` 타겟 끝에
`touch $(KERNEL_ELF)`를 추가해 해결.

**변경 내용:**

| 항목 | full | quick |
|---|---|---|
| `BENCH_TIMEOUT_TICKS` | 4000 (~222초) | 300 (~17초) |
| `N_SAMPLES` | 40 | 10 |
| `CFS2_TRIALS` | 3 | 1 |
| `DEMO_WAIT_TICKS` | 300 | 120 |
| `DEMO_LOG_SCALE` (로그 간격 배수) | 1 | 100 |
| `HOG_TARGET_ITERS` | 20,000,000 (공통 — 무관함이 확인됨) | |

로그 간격을 절대값이 아니라 **배수**로 둔 이유: full 모드에서 배수가 1이
되어 기존 간격(15/30/20건)이 그대로 유지된다. 로그량이 타이밍에 영향을 줄
수 있어 기존 실험을 돌렸던 조건을 바꾸지 않기 위함이다.

**발견 5 — 로그를 줄였더니 에뮬레이터가 포화됐다 (되돌아온 부작용):**

발견 3의 로그 감소 후 오히려 실행이 불안정해졌다. 같은 ISO로 한 번은
310초에 셸까지 갔는데, 다른 실행은 540초가 지나도 BETA-X 4에서 진행이
멈췄다. 원인을 추적하니 **타이머 틱 자체가 초당 18회에서 0.39회로
붕괴**해 있었다(496초 동안 192틱). kernel_main이 72틱을 기다리는데 틱이
안 도니 9분이 지나도 깨어나지 못한 것이다.

범인은 BETA-X 3/4의 생산자 태스크였다.

```rust
loop { send(); yield_now(); }   // 상한 없음
```

`yield_now()`는 소프트웨어 인터럽트를 거치는 완전한 컨텍스트 스위치라
TCG에서 매우 비싸다. 이걸 제한 없이 돌리면 에뮬레이터가 컨텍스트 스위치
처리에 포화되어 타이머 인터럽트를 제때 전달하지 못한다. IPC 카운트가
197만 회, 입력 이벤트가 236만 회까지 올라가 있었다.

**그리고 이 포화는 원래도 있었지만 로그가 가려주고 있었다.** 시리얼
출력은 바이트당 수십 µs를 바쁜 대기로 소모하므로, 촘촘한 로그가 사실상
루프의 속도 제한기 역할을 하고 있었다. 발견 3에서 로그를 100분의 1로
줄이자 그 제한이 사라지면서 잠재해 있던 포화가 드러난 것이다.
**성능 개선이 다른 곳의 잠재 문제를 노출시킨 사례다.**

**수정 — 틱 기반 페이싱:** 버스트마다 `sleep_ticks(1)`로 쉬게 했다.

```rust
if i % DEMO_BURST == 0 { sleep_ticks(1); }   // 200건마다 1틱 블로킹
else                   { yield_now(); }
```

틱 상한을 조절하는 대신 생산 속도를 타이머 속도에 묶는 방식이다. 틱이
느려지면 생산도 같이 느려지므로 생산자가 타이머를 앞지르는 상황 자체가
성립하지 않는다 — 자기 보정되는 구조라 상한값 튜닝보다 견고하다.
`sleep_ticks`(실험 31)는 `Blocked` 상태로 전환시켜 스케줄 대상에서 아예
빼므로 busy-wait이 아니다.

**Raw 데이터 (무부하, 페이싱 수정 후 4회 반복):**
...
- min/avg/max = **275 / 300 / 340초**, 범위 65초
- full(실험 42): 2,442초 → **평균 8.1배 단축**
- guest/wall 비율 약 1.0 (정상). 수정 전 정체 시에는 0.083이었다.
- **4회 전부 예외·panic 0건**
- 로그량: BETA-X 3/4 합계 104,545줄 → 2,006줄

수정 전에는 같은 ISO로 310초와 "540초 미도달"이 섞여 나왔으나, 수정 후
4회 편차가 65초 이내로 안정됐다.

**부수 관측 1 — 표본 부족:** quick 모드에서는 벤치가 항상 타임아웃까지
가므로 표본이 n=0~4로 적고, 한 번은 n=0이라 `avg=0cy`가 출력됐다.
"레이턴시 0사이클"로 오독될 수 있어, n=0이면 "표본 없음(측정 불가)",
n<N_SAMPLES면 "표본부족"을 명시하도록 출력을 수정했다. **quick 모드의
목적은 동작 확인이지 측정이 아님을 출력 자체가 드러내야 한다.**

**부수 관측 2 — 산발적 #DF:** 검증 중 IPC 데모 초입에서 `#DF Double Fault`가
1회 발생했으나 같은 ISO로 3회 재시도 시 재현되지 않았다. 실험 41에서 #DF에
IST2를 붙여둔 덕분에 트리플 폴트로 죽지 않고 리포트된 것으로 보인다.
#GP 계열과 같은 타이밍 의존 문제일 가능성이 있어 별도 관찰 대상으로 남긴다.

**부수 관측 3 — 호스트 부하 오염:** 중간 측정에서 guest 35초 / wall 481초로
14배 격차가 났다. 원인은 코드가 아니라 호스트에서 Minecraft(Java 서버
102% + 클라이언트 61%)가 돌고 있던 것이었다. QEMU TCG는 단일 스레드
CPU 바운드라 호스트 부하에 직격탄이다. 서버를 종료한 뒤에도 부하 평균이
가라앉을 때까지는 수치가 회복되지 않아, 짧은 구간(BETA-X 3 완료까지)을
2회 반복해 guest/wall 비율이 1.0으로 돌아온 것을 확인한 뒤에야 본 측정을
진행했다. **시간을 재기 전에 호스트 부하부터 확인하고, 비율이 1.0 근처인지
먼저 검증해야 한다.**

이 오염 때문에 한동안 "코드 회귀"를 의심하며 시간을 썼다. 위 발견 5의
진짜 원인(생산자 포화)에 도달한 것은, 호스트를 정리한 뒤에도 특정 구간
(BETA-X 4)에서만 재현된다는 점을 확인하고 나서였다. **환경 요인과 코드
요인이 겹쳐 있을 때는 환경부터 제거해야 원인이 보인다.**

**다음에 미친 영향:**
- 시연 영상 촬영(제출 필수 항목)의 선행 조건이 해소됐다.
- 검증 전문기관의 기능테스트에서 심사자가 41분을 기다리지 않아도 된다.
- README에 두 모드의 차이와 "quick 수치는 기록값과 비교 불가"를 명시.
- 방법론 교훈: (1) 구간을 좁혔다고 원인을 좁힌 것이 아니다 — 구간 안의
  원인은 따로 확인해야 한다. (2) wall clock과 guest time을 함께 봐야
  I/O 병목이 보인다. (3) 빌드 산출물이 실제로 갱신됐는지 확인하지 않으면
  측정 자체가 무효가 된다.
