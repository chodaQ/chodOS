...
**Raw 데이터:**
- full(실험 42): 셸 도달 2,442초, 로그 148,223줄
- quick: **셸 도달 310초**, 로그 9,282줄, panic/예외 0건 → **7.9배 단축**
...
**부수 관측 3 — 호스트 부하 오염:** 마지막 측정에서 guest 35초 / wall 481초로
14배 격차가 났다. 원인은 코드가 아니라 호스트에서 Minecraft(Java 서버
102% + 클라이언트 61%)가 돌고 있던 것이었다. QEMU TCG는 단일 스레드
CPU 바운드라 호스트 부하에 직격탄이다. **앞으로 시간 측정 시에는 호스트
부하를 먼저 확인해야 한다** — 310초라는 수치도 완전히 무부하 상태에서
재확인이 필요하다.
