...
/// BETA-X 3/4 데모 생산자 태스크의 버스트 크기 — 이 개수마다 1틱 쉰다.
///
/// 두 데모의 생산자(`wm_task`, `key_gen_task`)는 원래
/// `loop { send(); yield_now(); }` 형태로 무한정 이벤트를 만들었다.
/// `yield_now()`는 소프트웨어 인터럽트를 거치는 완전한 컨텍스트 스위치라
/// TCG에서 매우 비싼데, 이걸 제한 없이 돌리면 에뮬레이터가 컨텍스트 스위치
/// 처리에 포화되어 **타이머 인터럽트 자체가 거의 전달되지 않는다.**
/// 실측(실험 46)에서 틱 속도가 초당 18회에서 0.39회로 무너져,
/// 72틱을 기다리는 kernel_main이 9분이 지나도 깨어나지 못했다.
///
/// 버스트마다 `sleep_ticks(1)`로 쉬면 생산 속도가 타이머 속도에 묶이므로
/// 생산자가 타이머를 앞지르는 상황 자체가 성립하지 않는다. 관찰 목적
/// (임계값 돌파 → fast channel 생성/회수)에는 이 정도 트래픽이면 충분하다.
pub const DEMO_BURST: u64 = 200;
