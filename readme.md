...
> ⚠️ **알려진 이슈:** 현재 부팅 시퀀스 중 타이머/yield ISR의 `iretq`에서
> 간헐적으로 `#GP` fault가 발생하는 회귀가 있다. 코드 레벨 관측으로는
> 원인을 특정하지 못해(자세한 디버깅 과정은 `EXPERIMENTS.md` 참고) 현재
> 보류 중이며, 다음 우선순위 작업이다.
...
path를 만드는" 시스템 자체는 끝까지 동작시켰다. 통신 빈도를 관찰해 프로세스
쌍마다 fast path를 동적으로 만들고 없애는 이 구체적 방식은 조사한 범위
내에서는 못 찾았지만, 학계 전체를 다 훑은 건 아니라 "완전히 새롭다"고
단정하지는 않는다 — 컨텍스트 스위치가 마이크로커널 오버헤드의 핵심이라는
것 자체는 L4 계열 연구에서 이미 오래전부터 나온 얘기다. 그런데 3차례 측정
