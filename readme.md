...
**컨텍스트 스위치가 진짜 병목이었다.** 공유 메모리로 복사를 줄이는 접근은 거의
효과가 없거나 오히려 느렸다(최대 0.4×). 스케줄러를 거치지 않는 atomic 직접
통신은 32배 빨랐다. → 통신이 잦은 프로세스 쌍에만 전용 채널을 만들고 뜸해지면
회수하는 **동적 IPC fast path**를 설계.

**CFS를 기본 스케줄러로 올렸다가 되돌렸다.** 벤치마크에선 지연이 2배 개선됐지만
전체 데모에선 IPC 소비자가 굶어 큐가 26만 칸까지 불어나 OOM. 큐를 한 번에 비우는
소비자가 *일을 많이 했다는 이유로* vruntime 벌점을 받는 구조였다 — CPU 시간의
공정함이 진행의 공정함을 보장하지 않는다 ([실험 44~45](EXPERIMENTS.md)).

스케줄러는 가중치 기반과 CFS를 런타임에 전환할 수 있어 같은 워크로드에서 직접
비교할 수 있다.
