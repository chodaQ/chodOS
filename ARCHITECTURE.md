...
## 8. 다음 로드맵 (2026-07-27 기준)

> KSEF/대회 제출(1차 마감 8/27) 전까지 남은 작업을 우선순위 순으로
> 정리한다. 순서 기준은 "이 프로젝트를 논문/발표로 정리할 때 설명 못
> 하고 넘어가면 치명적인 것"부터. 별도로, 디시 프로그래밍 갤러리에
> 8/10경 재게시 예정 — 그 전까지 문서 정리는 사용자가 직접 진행.

### 8.0 해결된 항목 (요약 — 전체 근거는 EXPERIMENTS.md)

```
- #GP 부팅 회귀: 실험 40(QEMU 11.0.0→11.0.3 업데이트, 외부 요인)
  + 실험 41(NMI/#DF 전용 IST 스택, 내부 방어 조치)의 이중 방어 적용.
  실험 42에서 정상 타이밍 전체 데모(ALPHA~CFS-2~ELF~musl~셸)를 크래시
  0건으로 완주 재확인. "확정된 근본원인 수정"은 아니라는 한계는 계속
  명시(QEMU 11.0.0이 삭제돼 IST 추가의 직접 A/B 검증은 못 함).
- on_switch aliasing UB: 실험 43에서 do_preempt()의 재진입 aliasing을
  반환값 구조 변경으로 제거. 30분 회귀 부팅(CFS-1 완료까지)에서 이상
  없음 확인.
```

### 8.1 (최우선) 네트워크/파일시스템을 Policy Engine 관찰 대상으로 확장

```
디시 피드백("게임 모드에서 CPU만 성능모드 쓴다는데 네트워크는?
파일시스템은? GPU는?")에 대한 실제 답. VirtIO net/blk + TCP/IP + ext4가
이미 구현돼 있으므로, CPU/메모리에 하고 있는 것과 같은 패턴(관찰 →
분류 → 우선순위/전력 힌트 반영)을 네트워크·파일시스템 I/O에도 확장하는
것이 다음 목표. 사용자 확인(2026-07-27): "실사용에서 이렇다"고 주장하려면
실제로 커버리지를 넓혀야 가치가 있다는 판단.

GPU는 이 확장 대상에서 명시적으로 제외한다 — QEMU의 가상 프레임버퍼는
전력/성능 상태 자체가 없어 "게임 모드에서 GPU 성능모드" 같은 기능을
실제로 구현할 방법이 없다. 억지로 뭔가 만드는 것보다, 왜 스코프 밖인지
문서(README/논문)에 정직하게 남기는 쪽으로 결정(2026-07-27).
```

### 8.2 PE-3 lag-1 정량화 + ST-5 장기 A/B 재설계
...
핵심 질문에 아직 답을 못한 상태이므로, 시간이 되면 8.1보다 먼저
당겨도 됨(발표 임팩트가 큼).
```

### 8.3 실기(real hardware) 부팅 — 대회 이후로 명시적 보류

```
사용자 제안(2026-07-27)으로 검토했으나 스코프가 너무 큼: 지금 커널은
VirtIO(반가상화 전용) 블록/네트워크에 의존하는데, 실제 하드웨어로 가려면
AHCI/NVMe 같은 진짜 디스크 드라이버, 실제 NIC 드라이버(e1000e/realtek
등), 다양한 실기 UEFI 펌웨어 대응까지 전부 새로 만들어야 함 — VirtIO
스택 전체를 다시 짜는 것보다 큰 작업. KSEF 1차 마감(8/27) 전에 손대면
남은 시간을 전부 여기 태울 위험이 크다고 판단해 보류. 대회 이후의
장기 로드맵 항목으로만 기록.
```

### 8.4 GitHub Release 빌드 아티팩트 (ELF/ISO)

```
아카라이브 피드백("qemu 환경 맞추기보단 elf 파일 하나 있으면 좋을 거
같은데")에 대한 대응. 지금 바로 하기보다 8/10 프갤 재게시 직전에 맞춰
진행하는 게 타이밍상 낫다고 판단(사용자 확인, 2026-07-27) — 재게시
시점에 최신 안정 빌드를 같이 공개하는 흐름.
```

### 8.5 보류 (대회 제출 이후로 명시적으로 미룸)
...
```
8.1~8.2는 전부 "코드를 더 얹는" 작업이지만, MEMORY.md에 기록된 대로
